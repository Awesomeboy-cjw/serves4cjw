...
private:
    ...
    static const int USER_PER_PROCESS = 65530;  /* 每个子进程最多可以处理的客户数量 */
    static const int MAX_EVENT_NUMBER = 10000;  /* epoll最多能处理的时间数 */
    int m_process_number;    /* 进程池中进程总数 */
    int m_idx;               /* 子进程在进程池中的序号, 从0开始 */
    int m_epollfd;           /* 每个进程的epoll内核事件表 */
    int m_listenfd;          /* 每个子进程的监听描述符 */
    int m_stop;
    process* m_sub_process;  /* 指向所有子进程描述信息 */
